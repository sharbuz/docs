...
- `--values elasticsearch/values-aws.yaml` - Uses AWS-specific configuration
...
The PostgreSQL Operator automates the management of PostgreSQL databases in Kubernetes. While CodeMie uses a cloud-managed RDS PostgreSQL instance, the operator is required for Keycloak database configuration.
...
Get your RDS PostgreSQL connection details from the infrastructure deployment outputs:
