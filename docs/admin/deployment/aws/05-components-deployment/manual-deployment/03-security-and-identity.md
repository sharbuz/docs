...
- `-n security` - Deploys to the security namespace
- `--values keycloak-helm/values-aws.yaml` - Uses AWS-specific configuration including RDS PostgreSQL connection
...
```
https://keycloak.<your-domain>/auth/admin
```

**Example URLs**:

- `https://keycloak.codemie.example.com/auth/admin`
...
- `--values oauth2-proxy/values-aws.yaml` - Uses AWS-specific configuration
